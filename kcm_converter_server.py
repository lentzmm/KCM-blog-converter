...
def extract_images(original_html: str, converted_html: str, focus_keyphrase: str = "") -> List[Dict]:
    ...
    # Generate slug fragment for unique, SEO-rich filenames
    # Use first 2-4 words from slug to create descriptive filename
    # This prevents duplicates while maintaining SEO value
    ...
    # Extract slug fragment (first 2-4 meaningful words)
    # Remove common stop words to get to the meat of the title
    slug_words = [w for w in slug.split('-') if w not in ['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'is', 'are', 'was', 'were']]

    # Use first 2-4 words as the slug fragment
    if len(slug_words) >= 4:
        slug_fragment = '-'.join(slug_words[:4])  # e.g., "housing-market-going-crash"
    elif len(slug_words) >= 3:
        slug_fragment = '-'.join(slug_words[:3])  # e.g., "home-prices-flat"
    elif len(slug_words) >= 2:
        slug_fragment = '-'.join(slug_words[:2])  # e.g., "first-time"
    elif len(slug_words) == 1:
        slug_fragment = slug_words[0]  # e.g., "downsizing"
    else:
        slug_fragment = 'real-estate'  # Fallback

    # Limit slug fragment to reasonable length (25 chars max)
    if len(slug_fragment) > 25:
        # If too long, use first 3 words instead of 4
        slug_fragment = '-'.join(slug_words[:3]) if len(slug_words) >= 3 else slug_fragment[:25]

    logger.info(f"Slug fragment for images: {slug_fragment}, Geo term: {geo_term}")
    ...
    for index, match in enumerate(matches, 1):
        ...
        # Generate SEO/GEO optimized filename using slug fragment
        # Format: {slug-fragment}-{geo}-{descriptor}.ext
        # Example: housing-market-crash-south-jersey-guide.png
        # This ensures unique filenames while maintaining SEO value
        if index == 1:
            suggested_filename = f"{slug_fragment}-{geo_term}-guide{ext}"
        else:
            suggested_filename = f"{slug_fragment}-{geo_term}-{index}{ext}"
...
OUTPUT: Return ONLY the rewritten HTML. No preamble, no explanation, no code fences, just the complete localized blog post in HTML format ready for WordPress."""
    ...
    try:
        ...
        # Extract SEO metadata from Claude's response if present
        # Look for ===SEO_METADATA=== marker
        claude_seo_metadata = {}
        seo_marker_match = re.search(r'===SEO_METADATA===(.*?)===END_SEO_METADATA===', rewritten_html, re.DOTALL | re.IGNORECASE)

        if seo_marker_match:
            seo_section = seo_marker_match.group(1)
            logger.info("Found SEO metadata from Claude's conversion")

            # Extract keyphrase, SEO title, and meta description
            keyphrase_match = re.search(r'KEYPHRASE:\s*(.+)', seo_section, re.IGNORECASE)
            title_match = re.search(r'SEO_TITLE:\s*(.+)', seo_section, re.IGNORECASE)
            desc_match = re.search(r'META_DESCRIPTION:\s*(.+)', seo_section, re.IGNORECASE)

            if keyphrase_match:
                claude_seo_metadata['focus_keyphrase'] = keyphrase_match.group(1).strip()
                logger.info(f"  Keyphrase: {claude_seo_metadata['focus_keyphrase']}")
            if title_match:
                claude_seo_metadata['seo_title'] = title_match.group(1).strip()
                logger.info(f"  SEO Title: {claude_seo_metadata['seo_title']}")
            if desc_match:
                claude_seo_metadata['meta_description'] = desc_match.group(1).strip()
                logger.info(f"  Meta Desc: {claude_seo_metadata['meta_description'][:60]}...")

            # Remove the SEO metadata section from the HTML
            rewritten_html = rewritten_html[:seo_marker_match.start()]

        # Remove any other markdown sections at the END
        # Look for patterns like "## OUTPUT", "### 1.", "### 2.", etc.
        end_section_patterns = [
            r'\n#+\s*(OUTPUT|SEO|IMAGE|KEYPHRASE|DELIVERABLE|PART).*',
        ...
        return rewritten_html, claude_seo_metadata
...
def convert():
    ...
    try:
        ...
        # Rewrite blog post (now returns HTML and SEO metadata from Claude)
        converted_html, claude_seo_metadata = rewrite_blog_post(original_html, relevant_pages)

        if not converted_html:
            return jsonify({'error': 'Conversion failed'}), 500

        # Generate SEO metadata (AI-generated) as backup/fallback
        ...
        # Use Claude's SEO metadata from conversion if available, otherwise use AI-generated
        # Claude's metadata is preferred because it matches what's actually in the HTML
        seo_metadata['article_title'] = ai_seo_metadata.get('article_title', '')
        seo_metadata['focus_keyphrase'] = claude_seo_metadata.get('focus_keyphrase') or ai_seo_metadata.get('focus_keyphrase', '')
        seo_metadata['seo_title'] = claude_seo_metadata.get('seo_title') or ai_seo_metadata.get('seo_title', '')
        seo_metadata['meta_description'] = claude_seo_metadata.get('meta_description') or ai_seo_metadata.get('meta_description', '')

        if claude_seo_metadata:
            logger.info("Using SEO metadata from Claude's conversion (preferred)")
        else:
            logger.info("Using AI-generated SEO metadata (fallback)")
